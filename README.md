...
**Q: 下載後出現「無法驗證開發者」或「可能有害」警告？**

A: 這是 macOS Gatekeeper 對未公證 app 的標準提示。在終端機執行以下指令移除隔離屬性即可正常開啟：

```bash
xattr -rd com.apple.quarantine /Applications/TypelessMLX.app
```

或右鍵點 app → 選「打開」→ 再點「打開」確認。之後就不會再出現此警告。
